...
## Важно для Railway: версия Python

Если Railway автоматически выбирает Python 3.13, установка зависимостей (особенно `rembg` / `onnxruntime`) может падать на этапе `pip install`.

Что сделано в проекте:
- добавлены файлы `.python-version` (`3.11.14`) и `runtime.txt` (`python-3.11.14`) для фиксации Python 3.11;
- зависимости `rembg` и `onnxruntime` в `requirements.txt` ограничены маркером `python_version < "3.13"`, чтобы сборка проходила даже если Railpack оставит 3.13.

На Python 3.13 бот запустится и будет работать, а обработка фото автоматически перейдёт в fallback (использование оригинала), если `rembg` недоступен.

Если деплой уже падал, после коммита просто перезапустите redeploy.
